...
class LlamaLocal(AbstractLanguageModel):
    """
    An interface to use LLaMA models locally.
    """
    ...
    def __init__(
        ...
        """
        Initialize an instance of the LlamaLocal class with configuration, model details, and caching options.
    ...
    def query(self, query: str, num_responses: int = 1) -> List[Dict]:
        """
        Query the LLaMA model for responses.

        :param query: The query to be posed to the language model.
        :type query: str
        :param num_responses: Number of desired responses, default is 1.
        :type num_responses: int
        :return: Response(s) from the LLaMA model.
        :rtype: List[Dict]
        """
        if self.cache and query in self.response_cache:
            return self.response_cache[query]
        sequences = []
        query = f"<|begin_of_text|><|start_header_id|>system<|end_header_id|>You are a helpful assistant. Always follow the intstructions precisely and output the response exactly in the requested format.<|eot_id|>\n<|start_header_id|>user<|end_header_id|> {query} <|eot_id|>\n<|start_header_id|>assistant<|end_header_id|>"
        for _ in range(num_responses):
            self.logger.info(f'Generating response {_} of {num_responses}')
            sequences.extend(
                self.generate_text(
                    query,
                    do_sample=True,
        ...
        response = [
            {"generated_text": sequence["generated_text"][len(query) :]}
