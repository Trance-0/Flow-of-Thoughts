...
class Thought:
    ...
    def __init__(
        ...
        self.logger.debug(
            f"Parameters received: content={content}, parents_operations={parents_operations}, is_executable={is_executable}, children_operations={children_operations}"
        )
        ...
        self.logger.debug(
            f"Thought initialized with content: {self.content}, parents_operations: {','.join([str(pop) for pop in self.get_parents_operations()])}, is_executable: {self.is_executable}, children_operations: {','.join([str(cop) for cop in self.get_children_operations()])}"
        )
    ...
    def append_child_operation(self, child) -> None:
        ...
        if child.hash not in [op.hash for op in self.get_children_operations()]:
            ...
            self.logger.warning(
                "Hash collision detected for {child}, child already exists. Request rejected."
            )
    ...
    def __full_json__(self) -> dict:
        return {
            ...
            "operations": [op.__full_json__() for op in self.get_children_operations()],
        }

    def as_G6_node(self) -> dict:
        return {
            "id": hex(self.hash),
            "data": {
                ...
                "ori_operation": (
                    "root"
                    if len(self.get_parents_operations()) == 0
                    else self.get_parents_operations()[0].operation_type
                ),
                "is_executable": self.is_executable,
            },
            "states": ["active" if self.is_executable else "inactive"],
        }
    ...
    def __json__(self) -> dict:
        return {
            ...
            "hash": self.hash,
            "is_executable": self.is_executable,
            "operations": [op.hash for op in self.get_children_operations()],
        }
